...
import (
	...
	"github.com/santiagotorres/in-toto-webhook/pkg/webhook"
)
...
func NewFlags() *Flags {
	...
	fl.StringVar(&flags.ListenAddress, "listen-address",
		lAddressDef, "webhook server listen address")
	fl.StringVar(&flags.MetricsListenAddress,
		"metrics-listen-address", lMetricsAddress,
		"metrics server listen address")
	fl.BoolVar(&flags.Debug, "debug", debugDef,
		"enable debug mode")
	fl.StringVar(&flags.CertFile, "tls-cert-file",
		"certs/cert.pem", "TLS certificate file")
	fl.StringVar(&flags.KeyFile, "tls-key-file",
		"certs/key.pem", "TLS key file")
	fl.IntVar(&flags.MinScore, "min-score", 0,
		"Kubesec.io minimum score to validate against")
...
func (m *Main) Run() error {
	...
	// Create webhooks
	pw, err := webhook.NewPodWebhook(m.flags.MinScore,
		metricsRec, m.logger)
	if err != nil {
		return err
	}
	...
	vdw, err := webhook.NewDeploymentWebhook(m.flags.MinScore,
		metricsRec, m.logger)
	if err != nil {
		return err
	}
	...
	dw, err := webhook.NewDaemonSetWebhook(m.flags.MinScore,
		metricsRec, m.logger)
	if err != nil {
		return err
	}
	...
	sw, err := webhook.NewStatefulSetWebhook(m.flags.MinScore,
		metricsRec, m.logger)
	if err != nil {
		return err
	}
	...
	go func() {
		...
		mux.HandleFunc("/links/", metadatastore.HandleLinkSerialization)
